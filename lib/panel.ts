class Panel {
  ...
  constructor(name: string, fg: string, bg: string, staticMax = false) {
  ...
  update(value: number, valueGraph: number, maxValue: number, maxGraph: number, decimals = 0) {
      ...
      max = this.staticMax ? this.max : Math.max(maxValue, value);
      maxGraph = this.staticMax ? this.max : Math.max(maxGraph, valueGraph);
